...
if zip_RTS is not None:
    ...
    with st.sidebar:
        ...
        def df_to_xlsx(df_Tenant,df_Search):
            byte_xlsx=BytesIO()
            ...
            df_Tenant.drop('View',axis=1).to_excel(writer_xlsx,index=False,sheet_name="検索結果 | Search Result")
            writer_xlsx.close()
            out_xlsx=byte_xlsx.getvalue()
            return out_xlsx
        out_xlsx=df_to_xlsx(df_Tenant,df_Search)
        st.download_button('📋 Download Excel',out_xlsx,file_name='TenantSearch.xlsx')